...
const Login = () => {
  ...
  if(user && name){
    ...
    async function handleSubmit(){
      ...
      else{setValues(pre=>({...pre,error:'Invalid Mail Address'}))}
      }
    }
    else{
      if(values.name && values.username && values.email && values.pass && values.Des){
  ...
  return (
    <main>
      ...
      <div id='ball_3'></div>
      {values.loading?<div id="loading_animation">
      ...
      <p style={{fontSize:'1.3rem',color:'#f9ff00'}}>please wait...</p>
        ...
        <div id='login'>
          ...
          <>
           ...
           <div id='input'>
           <input type='text' onChange={(e)=>setValues(pre=>({...pre,Des:e.target.value}))} placeholder='Description' />
           </div>
           </>:<></>}
             <div id='input'>
            ...
            <div id='Submit_form'>
            <p style={{color:'red',textAlign:'center',fontSize:'.8rem'}}>{values.error?values.error:' '}</p>
